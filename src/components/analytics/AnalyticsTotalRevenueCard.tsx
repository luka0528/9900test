...
export const AnalyticsTotalRevenueCard = () => {
  ...
  const [currMonthQuery, prevMonthQuery] = api.useQueries((trpc) => [
    trpc.analytics.getTotalRevenue({
  ...
  const { data: currMonthRevenue, isLoading: isCurrMonthLoading } =
    currMonthQuery;

  const { data: prevMonthRevenue, isLoading: isPrevMonthLoading } =
    prevMonthQuery;

  const revenueDelta =
    currMonthRevenue && prevMonthRevenue
