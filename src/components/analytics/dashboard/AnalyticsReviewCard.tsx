...
export const AnalyticsReviewCard = ({ review }: AnalyticsReviewCardProps) => {
  ...
  const formattedDate = formatDistanceToNow(review.createdAt, { addSuffix: true })
  ...
  return (
    <Card className="p-4 w-full">
      <div className="flex items-start justify-between">
        ...
        <a href={`/reviews/${review.id}`} className="cursor-pointer text-gray-400 hover:text-gray-600">
          <ExternalLink size={18} />
        </a>
      </div>
      <div className="mt-2 text-xs line-clamp-2">{review.content}</div>
    </Card>
  )
}
