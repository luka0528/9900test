...
export function NotificationDropdown() {
  ...
  const { mutate: deleteNotification } =
    api.notification.deleteNotification.useMutation({
      onMutate: async ({ notificationId }) => {
        ...
        const prevData = utils.notification.getMyNotifications.getData({
          includeRead: true,
          limit: 50,
        });
  ...
  const handleDelete = useCallback(
    (notificationId: string, e: React.MouseEvent) => {
      e.stopPropagation();
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      className="h-7 w-7 rounded-full p-0 hover:bg-destructive/10 hover:text-destructive"
