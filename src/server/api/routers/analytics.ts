...
export const analyticsRouter = createTRPCRouter({
  ...
  getRevenueOverTimeByService: protectedProcedure
    .query(async ({ ctx }) => {
        const receipts = await ctx.db.billingReceipt.findMany({
            where: {
              to: {
        ...
        const revenueOverTime = Array.from(groupedByDate.entries()).map(([date, services]) => {
            const revenue = Array.from(services.entries()).map(([serviceName, amount]) => ({
                serviceName,
                amount,
            }));
